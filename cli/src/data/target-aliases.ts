...
export interface TargetInfo {
  officialName: string;
  aliases: string[];           // All name variants (case-insensitive matching)
  geneSymbol: string;
  uniprotId?: string;
  commonMechanisms: string[];  // Phrases that indicate targeting this protein
  relatedTargets?: string[];   // For bispecifics (e.g., TL1A x IL-23)
}
...
export const TARGET_ALIASES: Record<string, TargetInfo> = {
  // ========== TL1A / TNFSF15 ==========
  'TL1A': {
    officialName: 'TL1A',
 ...
 */
export const COMMON_NON_TARGET_DRUGS: string[] = [
  // Chemotherapy
 ...
 */
export function getTargetInfo(target: string): TargetInfo | null {
  const normalized = target.toUpperCase().replace(/[-\s]/g, '');

  // Direct lookup
  for (const [key, info] of Object.entries(TARGET_ALIASES)) {
 ...
 */
export function getTargetVariants(target: string): string[] {
 ...
 */
export function isCommonNonTargetDrug(drugName: string): boolean {
  const normalized = drugName.toLowerCase().trim();

  return COMMON_NON_TARGET_DRUGS.some(drug => {
 ...
 */
export function inferTargetInfo(target: string): TargetInfo {
  return {
    officialName: target,
    aliases: [
      target,
