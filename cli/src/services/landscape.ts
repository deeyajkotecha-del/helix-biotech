...
export interface KOL {
  ...
  recentPublications: number; // last 3 years
  hIndex: number | null;
}
...
async function fetchClinicalTrials(condition: string): Promise<{
  trials: ClinicalTrial[];
  phaseBreakdown: Record<string, number>;
  topSponsors: { name: string; count: number }[];
  statusBreakdown: Record<string, number>;
  ...
  try {
    ...
    const trials: ClinicalTrial[] = allStudies.map((study: any) => {
      ...
      return {
        ...
        locations: locations.slice(0, 3).map((loc: any) => loc.country || 'Unknown'),
        interventions,
        primaryEndpoint,
...
function extractMolecules(trials: ClinicalTrial[]): Molecule[] {
  ...
  for (const trial of trials) {
    for (const intervention of trial.interventions) {
      ...
      if (isControlIntervention(normalizedName)) continue;
...
function extractMechanism(name: string): string | null {
  const lower = name.toLowerCase();
  if (lower.includes('mab') || lower.includes('umab') || lower.includes('zumab')) return 'Monoclonal Antibody';
  if (lower.includes('nib') || lower.includes('tinib')) return 'Kinase Inhibitor';
  if (lower.includes('ciclib')) return 'CDK Inhibitor';
  if (lower.includes('stat')) return 'JAK/STAT Inhibitor';
  if (lower.includes('car-t') || lower.includes('car t')) return 'CAR-T Cell Therapy';
  if (lower.includes('vaccine')) return 'Vaccine';
  if (lower.includes('sirna') || lower.includes('antisense')) return 'RNA Therapeutic';
  if (lower.includes('gene therapy')) return 'Gene Therapy';
  if (lower.includes('stem cell')) return 'Cell Therapy';
  return null;
}

function getPhaseRank(phase: string): number {
  const p = phase.toLowerCase();
  if (p.includes('approved') || p.includes('marketed') || p.includes('4')) return 100;
...
async function fetchDealsNews(condition: string): Promise<DealNews[]> {
...
function generateKeywordVariations(condition: string): string[] {
...
function parseRSSItems(xml: string, source: string, keywords: string[]): DealNews[] {
  const items: DealNews[] = [];
  ...
  for (const itemXml of itemMatches) {
    ...
    if (!matches && !isDealNews) continue;
...
function extractDealType(text: string): string | null {
  const lower = text.toLowerCase();
  if (lower.includes('acquisition') || lower.includes('acquires') || lower.includes('acquire') || lower.includes('buy')) return 'Acquisition';
  if (lower.includes('merger') || lower.includes('merge')) return 'Merger';
  if (lower.includes('partnership') || lower.includes('partner')) return 'Partnership';
  if (lower.includes('collaboration') || lower.includes('collaborate')) return 'Collaboration';
  if (lower.includes('licensing') || lower.includes('license')) return 'Licensing';
  if (lower.includes('investment') || lower.includes('invests')) return 'Investment';
  if (lower.includes('ipo') || lower.includes('initial public')) return 'IPO';
  if (lower.includes('funding') || lower.includes('raises') || lower.includes('series')) return 'Funding';
  if (lower.includes('fda approval') || lower.includes('approved') || lower.includes('green light')) return 'FDA Approval';
  if (lower.includes('phase 3') || lower.includes('phase 2') || lower.includes('pivotal')) return 'Clinical Update';
  if (lower.includes('data') || lower.includes('results') || lower.includes('readout')) return 'Data Readout';
  return null;
}
...
function extractCompanies(title: string): string[] {
  ...
  if (current) companies.push(current.trim());
...
async function fetchPubMedData(condition: string): Promise<{
  totalCount: number;
  byYear: { year: number; count: number }[];
  topKOLs: KOL[];
}> {
  ...
  try {
    ...
    let totalCount = 0;
    if (countResponse.ok) {
      const countData = await countResponse.json() as { esearchresult?: { count?: string } };
...
async function fetchYearCount(condition: string, year: number): Promise<number> {
...
async function fetchTopKOLs(condition: string, currentYear: number): Promise<KOL[]> {
  ...
  try {
    ...
    for (let i = 0; i < Math.min(idList.length, 300); i += 50) {
      ...
      for (const articleXml of articleMatches) {
        ...
        const authorsToCount: string[] = [];
        if (authorMatches.length > 2) {
          if (authorMatches[0]) authorsToCount.push(authorMatches[0]);
          const lastAuthor = authorMatches[authorMatches.length - 1];
          if (lastAuthor) authorsToCount.push(lastAuthor);
        } else {
          authorsToCount.push(...authorMatches.filter((a): a is string => !!a));
        }
...
const authorCountCache = new Map<string, number>();
 ...
 */
async function getAuthorPublicationCount(
  authorName: string,
  condition: string,
  yearsBack?: number
): Promise<number> {
...
function extractInstitution(affiliation: string): string | null {
...
export async function getLandscapeData(condition: string): Promise<LandscapeData> {
  // Check cache first
...
function getCacheFilePath(condition: string): string {
  const sanitized = condition.toLowerCase().replace(/[^a-z0-9]+/g, '-');
  return path.join(CACHE_DIR, `${sanitized}.json`);
}

function getCachedLandscape(condition: string): LandscapeData | null {
  const filePath = getCacheFilePath(condition);
  if (!fs.existsSync(filePath)) return null;

  try {
    const data = fs.readFileSync(filePath, 'utf-8');
    const cached = JSON.parse(data) as LandscapeData;
...
function saveCachedLandscape(condition: string, data: LandscapeData): void {
...
export function generateLandscapeCSV(data: LandscapeData): string {
  const lines: string[] = [];
