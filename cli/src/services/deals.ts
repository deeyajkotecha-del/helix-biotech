 ...
 */
export async function searchDealsByCompany(companyName: string): Promise<Deal[]> {
 ...
 */
export async function searchDealsByAsset(assetName: string): Promise<Deal[]> {
  // TODO: Implement
  throw new Error('Not implemented');
}
 ...
 */
export async function getDealsByTherapeuticArea(
 ...
 */
export async function getMAActivity(
  year: number
): Promise<{
  totalDeals: number;
  totalValue: number;
 ...
 */
export function extractDealInfo(title: string, description: string): Partial<Deal> {
 ...
 */
export function extractDealType(text: string): DealType | null {
  const lower = text.toLowerCase();

  if (lower.includes('acquisition') || lower.includes('acquires') || lower.includes('to buy')) return 'Acquisition';
  if (lower.includes('merger') || lower.includes('merge')) return 'Merger';
  if (lower.includes('license') && !lower.includes('sublicense')) return 'Licensing';
  ...
  if (lower.includes('co-develop') || lower.includes('codevelop')) return 'Co-development';
  if (lower.includes('option agreement') || lower.includes('option to')) return 'Option';
  if (lower.includes('asset purchase') || lower.includes('asset sale')) return 'Asset Purchase';
  if (lower.includes('series ') || lower.includes('funding') || lower.includes('raises $')) return 'Funding';
 ...
 */
export function extractUpfrontPayment(text: string): number | undefined {
  const patterns = [
...
export function extractMilestones(text: string): number | undefined {
  const patterns = [
...
export function extractTotalValue(text: string): number | undefined {
  const patterns = [
...
export function extractRoyalties(text: string): string | undefined {
  const patterns = [
 ...
 */
export function extractAssetInfo(text: string): Deal['asset'] | undefined {
 ...
 */
export async function searchSECMaterialAgreements(
  companyName: string,
  options?: {
    daysBack?: number;
  }
