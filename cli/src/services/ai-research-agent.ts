...
import { KnownAsset, RegulatoryDesignations, DealTerms } from '../data/known-assets';
...
function buildResearchPrompt(target: string): string {
...
function getSearchQueriesUsed(target: string): string[] {
  return [
...
function parseAssetsFromResponse(response: Anthropic.Message, target: string): DiscoveredAsset[] {
...
function normalizeAsset(raw: any, target: string): DiscoveredAsset | null {
  if (!raw || typeof raw !== 'object') return null;
  if (!raw.name && !raw.drugName && !raw.primaryName) return null;
...
function normalizePhase(phase: string): string {
  ...
  if (p.includes('filed') || p.includes('bla') || p.includes('nda')) return 'Filed';
  if (p.includes('3')) return 'Phase 3';
  if (p.includes('2/3') || p.includes('2-3')) return 'Phase 2/3';
  if (p.includes('2')) return 'Phase 2';
  if (p.includes('1/2') || p.includes('1-2')) return 'Phase 1/2';
  if (p.includes('1')) return 'Phase 1';
  if (p.includes('preclin')) return 'Preclinical';
  return phase;
}

function normalizeConfidence(conf: string | undefined): 'HIGH' | 'MEDIUM' | 'LOW' {
...
function isValidAsset(asset: DiscoveredAsset): boolean {
  // Basic validation
  if (!asset.name || asset.name.length < 2) return false;
  if (asset.name.toLowerCase() === 'unknown') return false;
  ...
  for (const excluded of excludedDrugs) {
    if (nameLower.includes(excluded)) return false;
  }

  return true;
}

function extractAssetsFromText(text: string, target: string): DiscoveredAsset[] {
...
function countSourcesFromResponse(response: Anthropic.Message): number {
  let count = 0;
  for (const block of response.content) {
    if (block.type === 'text') {
 ...
 */
export function convertToKnownAssets(discovered: DiscoveredAsset[]): KnownAsset[] {
  return discovered.map(d => {
    const regulatory: RegulatoryDesignations = {
      btd: false,
      odd: false,
      fastTrack: false,
      prime: false,
    ...
    const deal: DealTerms | undefined = d.deal ? {
    ...
    return {
      ...
      partner: d.partner,
      phase: d.phase as KnownAsset['phase'],
      status: (d.status === 'Active' ? 'Active' : 'On Hold') as KnownAsset['status'],
...
function normalizeModality(modality: string): KnownAsset['modality'] {
  const m = modality.toLowerCase();
  if (m.includes('adc') || m.includes('antibody drug conjugate')) return 'ADC';
  if (m.includes('bispecific') || m.includes('bi-specific')) return 'Bispecific';
  if (m.includes('car-t') || m.includes('car t')) return 'CAR-T';
  if (m.includes('bite')) return 'BiTE';
  if (m.includes('small molecule')) return 'Small Molecule';
  if (m.includes('radio')) return 'Radioconjugate';
  if (m.includes('vaccine')) return 'Vaccine';
  if (m.includes('mab') || m.includes('monoclonal')) return 'mAb';
  return 'Other';
}

function normalizeOwnerType(ownerType: string | undefined): KnownAsset['ownerType'] {
  if (!ownerType) return 'Biotech';
  const o = ownerType.toLowerCase();
  if (o.includes('big pharma') || o.includes('large pharma')) return 'Big Pharma';
  if (o.includes('chinese')) return 'Chinese Biotech';
  if (o.includes('academic') || o.includes('university') || o.includes('institute')) return 'Academic';
  return 'Biotech';
}
