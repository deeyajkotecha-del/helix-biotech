...
import {
  getTargetModalities,
  getComprehensiveSearchTerms,
  hasCuratedModalities,
} from '../data/target-modalities';
import {
  getTargetInfo,
...
import { discoverAssets, VerifiedAsset } from './asset-research';
 ...
 */
async function phase1BroadDiscovery(target: string): Promise<string[]> {
  const searchTerms = [
 ...
 */
async function phase2ModalitySearches(target: string): Promise<string[]> {
  const modalities = getTargetModalities(target);
  const searchTerms = modalities.relevantModalities.map(mod =>
    `${target} ${mod} clinical development`
  );
 ...
 */
async function phase3GeographicSearches(target: string): Promise<string[]> {
  const searchTerms = [
 ...
 */
async function phase4AcademicSearches(target: string): Promise<string[]> {
  const searchTerms = [
 ...
 */
async function phase5DealSearches(target: string): Promise<string[]> {
  const searchTerms = [
 ...
 */
async function phase6CompanySearches(target: string): Promise<string[]> {
  const modalities = getTargetModalities(target);
  const searchTerms = modalities.companyKeywords.slice(0, 8).map(company =>
 ...
 */
export async function comprehensiveTargetResearch(
  target: string
  ...
  const hasCurated = hasCuratedModalities(target);
  ...
  const trialDiscovery = await discoverAssets(target);

  // Convert curated assets to VerifiedAsset format
  const curatedVerified: VerifiedAsset[] = curatedAssets.map(asset => ({
    ...
    phase: asset.phase,
    status: asset.status === 'Active' ? 'Active' as const : 'Unknown' as const,
  ...
  let coverageLevel: 'comprehensive' | 'limited' | 'minimal';
  ...
  const result: ComprehensiveResearchResult = {
    ...
    discoveredAssets: trialDiscovery.verified.filter(
      a => a.verificationMethod !== 'curated_database'
    ),
 ...
 */
export function getResearchStatusMessage(target: string): {
